...
const Comments: React.FC = () => {
    ...
    const { token, login, is_avatar } = useSelector((state: StoreProps) => state.login);

    const query = useInfiniteQuery({
        queryKey: [ "comments", id, order ],
        initialPageParam: 0,
        queryFn: async ({ pageParam }) => {
            const response = await comments_list(Number(id), pageParam, undefined, order);
            return response;
        },
    ...
    const like = useMutation({
        ...
        onSuccess: (data, params) => {
            ...
            queryClient.setQueryData(["comments", id, order], ({ pages, pageParams }) => {
                const recursiveFind = (obj: CommentProps, visited = new Set<number>()) => {
    ...
    const del = useMutation({
        ...
        onSuccess: (data, params) => {
            ...
            notify(t(`notify.${is_deleted ? "deleted.comment" : "denied"}`), is_deleted ? "success" : "warning");
            queryClient.setQueryData(["comments", id, order], ({ pages, pageParams }) => {
                const recursiveFind = (obj: CommentProps, visited = new Set<number>()) => {
                    if (visited.has(obj.id)) return false;
                    visited.add(obj.id);
    ...
    const comments = [].concat(...(query.data?.pages?.map(page => page?.items) || []));
                ...
                <div className="flex flex-col gap-4 w-full mb-8">
                    ...
                    {comments?.map((comment, i) => (
                        <Comment
                            ...
                            onOk={(comment_id, comment) => create.mutate({ comment_id, comment })}
                            onLoadNext={(offset) => loadNext.mutate({ commentid: comment.id, offset })}
                            onDelete={(comment_id, comment_text) => {
                                modal(() => (
                                    <div className="flex flex-col gap-2">
                                        <p className="text-placeholder">{t("games.comments.confirm")}</p>
