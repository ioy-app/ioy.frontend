...
const Jams: React.FC = () => {
    ...
    for (let i = 0; i < days; i++) {
        const jams = query?.data?.items?.filter(jam => {
            const isValid = dayjs(dayjs(date_from).date(i + 1)).isBetween(jam.date_started, jam.date_finished);
                    ...
                    >
                        ...
                        {node?.jams?.slice(0, 3)?.map((jam, i) => (
                            <div className={`w-full h-6 bg-primary flex justify-center items-center nth-[2n]:bg-second`}>
                                <p className="text-white text-[10pt] w-[60%] overflow-hidden text-nowrap truncate">{jam.title}</p>
                            </div>
                        ))}
                        </div>
                    </div>
