	...
	return (
		...
		>
			<div className="h-full">
				{![paths.jams.list, "/"].includes(location.pathname) && (
					<NavLink to="/" className="h-full aspect-square p-0 m-0">
						<img src={Icons.Logo} className="aspect-square h-full" />
					</NavLink>
				)}
			</div>
			<nav className="h-full flex gap-4 items-center justify-end px-2">
				{((location.pathname != user_paths.details(login) && token) ||
					!token) && (
					<button
